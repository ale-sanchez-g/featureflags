...
run_with_docker() {
  ...
  docker run --rm -v "$REPO_ROOT":/src openpolicyagent/opa eval -i "$NOMATCH_INPUT" --data "$POLICIES_DIR" "data.pr_has_issue.allow" --fail-defined --format pretty --explain full 
