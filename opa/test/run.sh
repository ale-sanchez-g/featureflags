...
run_with_local_opa() {
  ...
  opa eval -i "$HOST_MATCH" --data "$HOST_POLICIES" "data.pr_has_issue.allow" --format pretty
  opa eval -i "$HOST_NOMATCH" --data "$HOST_POLICIES" "data.pr_has_issue.allow" --format pretty
}
