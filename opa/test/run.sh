...
run_with_docker() {
  ...
  docker run --rm -v "$REPO_ROOT":/src openpolicyagent/opa eval -i "$MATCH_INPUT" --data "$POLICIES_DIR" "data.pr_has_issue.allow" --format pretty || true
  docker run --rm -v "$REPO_ROOT":/src openpolicyagent/opa eval -i "$NOMATCH_INPUT" --data "$POLICIES_DIR" "data.pr_has_issue.allow" --format pretty || true
}

run_with_local_opa() {
  echo "Running tests with local opa binary..."
  opa eval -i "$HOST_MATCH" --data "$HOST_POLICIES" "data.pr_has_issue.allow" --format pretty || true
  opa eval -i "$HOST_NOMATCH" --data "$HOST_POLICIES" "data.pr_has_issue.allow" --format pretty || true
}
